import type { EnglishCatalog } from "../en/index.js";

/** Chinese shell copy aligned to local Codex (zh-CN) desktop chrome. */
export const zhCN = {
  app: {
    name: "PI-Desktop",
    shellName: "Codex",
    tagline: "本地优先的 AI 编程助手",
    starting: "正在启动 PI-Desktop…",
  },
  nav: {
    newTask: "新建任务",
    projects: "项目",
    pullRequests: "拉取请求",
    scheduled: "已安排",
    plugins: "插件",
    recents: "最近",
    settings: "设置",
    custom: "Custom",
    profileLogs: "日志",
    profileTheme: "主题",
    openProfileMenu: "打开个人菜单",
    search: "搜索",
    noRecentTasks: "没有最近任务",
    newChat: "新对话",
    sessions: "会话",
    commandPalette: "命令面板",
  },
  chat: {
    emptyTitle: "What should we build?",
    emptyTitleInProject: "What should we build in {{project}}?",
    emptyHint: "配置模型提供方、打开项目，然后发送第一条消息。",
    placeholder: "向 Codex 下达任意指令",
    addFiles: "添加文件等内容",
    attachFiles: "附加文件和文件夹",
    addPhotos: "添加照片",
    captureAppshot: "附加应用快照",
    filesAttached: "已将 {{count}} 个路径加入草稿",
    appshotSoon: "应用截图功能尚未接入",
    send: "发送",
    abort: "停止",
    running: "处理中…",
    noModel: "请先在设置中配置模型。",
    local: "本地",
    branch: "分支",
    agent: "代理",
    requestApproval: "请求批准",
    effortCustom: "自定义",
    effortLow: "轻度",
    effortMid: "中",
    effortHigh: "高",
    effortMax: "最高",
    untitledTask: "新建任务",
  },
  onboarding: {
    title: "开始使用",
    addProvider: "添加模型提供方",
    saveKey: "保存 API 密钥",
    openProject: "打开项目文件夹",
    firstPrompt: "发送第一条提示",
    loadPlugin: "加载开发插件（可选）",
  },
  settings: {
    title: "设置",
    providers: "提供方",
    models: "模型",
    permissions: "权限",
    appearance: "外观",
    plugins: "插件",
    about: "关于",
    addProvider: "添加提供方",
    apiKey: "API 密钥",
    baseUrl: "Base URL",
    modelId: "默认模型",
    save: "保存",
    testConnection: "测试连接",
    hasSecret: "已保存 API 密钥",
    noSecret: "未设置 API 密钥",
    delete: "删除",
    theme: "主题",
    mode: "默认模式",
    back: "返回",

    general: "常规",
    configuration: "配置",
    personalization: "个性化",
    voice: "语音",
    keyboard: "键盘快捷键",
    account: "账户",
    mcp: "MCP 服务器",
    browser: "浏览器",
    computer: "电脑操控",
    hooks: "钩子",
    connections: "连接",
    git: "Git",
    groupPersonal: "个人",
    groupIntegrations: "集成",
    groupCoding: "编码",
    backToApp: "返回应用",
    search: "搜索设置",
    searchPlaceholder: "搜索设置…",
    noResults: "未找到结果",
    enterToSend: "回车发送",
    enterToSendDesc: "关闭后使用 ⌘/Ctrl+Enter 发送。",
    modeDesc: "新建任务时使用的模式。",
    modeAgent: "Agent",
    modeChat: "Chat",
    yes: "是",
    noCmdEnter: "否（⌘/Ctrl+Enter）",
    themeDesc: "跟随系统或强制浅色/深色。",
    themeSystem: "系统",
    themeLight: "浅色",
    themeDark: "深色",
    defaultPermissions: "默认权限",
    defaultPermissionsDesc: "默认情况下，助手可以读取和编辑其工作空间中的文件。需要时，它可以请求额外访问权限。",
    autoReview: "自动审核",
    autoReviewDesc: "自动审核额外访问权限请求。自动审查可能会出错。",
    fullAccess: "完全访问权限",
    fullAccessDesc: "以完整访问权限运行时，无需你的批准即可编辑电脑上的任何文件，并运行可访问网络的命令。",
    showMenuBar: "在菜单栏中显示",
    showMenuBarDesc: "关闭主窗口后，仍在 macOS 菜单栏中保留应用。",
    modelIdDesc: "使用当前提供方创建新会话时的默认模型。",
    name: "名称",
    providerSaved: "提供方已保存",
    saving: "保存中…",
    saveProvider: "保存提供方",
    configured: "已配置",
    noProviders: "暂无提供方",
    noModel: "无模型",
    default: "默认",
    makeDefault: "设为默认",
    defaultUpdated: "默认提供方已更新",
    providerRemoved: "提供方已删除",
    installedPlugins: "已安装插件",
    loadDevPlugin: "加载开发插件",
    devPluginLoaded: "开发插件已加载",
    noPlugins: "暂无插件",
    enabled: "已启用",
    disabled: "已禁用",
    enable: "启用",
    disable: "禁用",
    uninstall: "卸载",
    application: "应用",
    applicationDesc: "产品标识与版本信息。",
    logs: "日志",
    logsDesc: "打开本地日志目录以便诊断。",
    openLogs: "打开日志",
    placeholderBody: "此设置分区已按 Codex 布局搭好。对应主机能力就绪后会接入真实控件。",
  },
  project: {
    open: "打开项目",
    none: "无项目",
    clear: "关闭项目",
    title: "项目",
    subtitle: "打开本地文件夹作为当前编程工作区。",
    add: "添加新项目",
    emptyTitle: "暂无项目",
    emptyBody: "添加本地项目文件夹后，工具、输入框上下文和首页标题都会绑定到该工作区。",
    active: "当前",
    switch: "切换",
    close: "关闭",
    pin: "置顶项目",
    unpin: "取消置顶",
    remove: "移除",
    branch: "分支",
  },
  pulls: {
    title: "拉取请求",
    subtitle: "在当前项目中查看和处理拉取请求。",
    refresh: "刷新",
    emptyTitle: "暂无拉取请求",
    emptyBody: "请先打开项目，然后让助手列出或审查拉取请求。",
    emptyBodyWithProject:
      "未检测到 {{project}} 的打开拉取请求。可新建任务检查 git 状态、分支或起草 PR。",
    review: "用助手审查",
    filters: "拉取请求筛选",
    filterOpen: "打开",
    filterDraft: "草稿",
    filterAll: "全部",
    open: "打开",
    draft: "草稿",
  },
  scheduled: {
    title: "已安排",
    subtitle: "自动化与周期任务。",
    emptyTitle: "暂无已安排任务",
    emptyBody:
      "创建带提示词的定时任务。当前支持立即运行；周期标签用于本地标记预期频率。",
    create: "创建任务",
    prompt: "提示词",
    cadence: "周期",
    cadenceManual: "手动",
    cadenceHourly: "每小时",
    cadenceDaily: "每天",
    cadenceWeekly: "每周",
    enabled: "已启用",
    disabled: "已禁用",
    runNow: "立即运行",
    delete: "删除",
    lastRun: "上次运行",
    never: "从未",
    tasks: "任务",
  },
  permission: {
    title: "需要权限",
    allowOnce: "允许一次",
    allowSession: "本会话允许",
    deny: "拒绝",
    riskHigh: "高风险",
    riskMedium: "中风险",
    riskLow: "低风险",
  },
  plugins: {
    title: "插件",
    subtitle: "通过本地开发插件扩展桌面助手。",
    loadDev: "加载开发插件",
    enable: "启用",
    disable: "禁用",
    uninstall: "卸载",
    empty: "未安装插件",
    emptyBody: "为你的工作添加插件。加载本地插件目录以注册命令与工具扩展。",
  },
  status: {
    ready: "就绪",
    degraded: "降级",
    hostOk: "Host 已连接",
    hostDown: "Host 不可用",
  },
  home: {
    suggestions: {
      explore: "探索并理解代码",
      explorePrompt: "探索",
      create: "构建新功能、应用或工具",
      createPrompt: "构建",
      review: "审查代码并提出修改建议",
      reviewPrompt: "审查",
      fix: "修复问题和失败",
      fixPrompt: "修复",
    },
  },
  errors: {
    HOST_UNAVAILABLE: "Host core 不可用",
    MODEL_NOT_CONFIGURED: "尚未配置模型",
    TOOL_DENIED: "工具权限被拒绝",
    PROVIDER_SECRET_MISSING: "缺少提供方 API 密钥",
  },
} satisfies EnglishCatalog;

export default zhCN;
