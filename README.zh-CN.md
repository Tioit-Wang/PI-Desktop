...
- **智能体与对话两种模式。** 智能体模式可以读写文件、执行命令，把事情做完；对话模式严格只读，适合放心地向它了解一个陌生代码库。
...
3. **描述任务。** 想改代码就用智能体模式，只想了解代码就用只读的对话模式。改动和命令逐条批准，提交前先在**审阅**面板里核对 diff。
...
PI-Desktop 处于活跃开发中的早期预览阶段。已交付：应用外壳、流式对话运行时、带权限系统的工作区智能体工具、插件基础设施，以及带更新检查的 macOS 打包。
