...
| Edit 契约 | 行锚定操作 + 整文件 `tag`；不再有 `old_string`/`new_string`（ADR 0089） |
...
代表的权利是其定义的，而不是其会话的。它无法获得
工具，因为父级拥有它，并且会话不能将突变权借给
只读委托。委托调用由会话运行时构建并进行
通过相同的 `tools.execute` 路径，因此路径规则 (§4)、Bash 规则 (§5)、
许可模式 (§6)、操作模式矩阵 (§10) 和审计 (§9) 适用
未更改 — 根据会话进行评估，因为会话拥有
工作空间和权限模式。

定义还可以声明 `permission: inherit | ask | accept-edits | auto`
（ADR 0089，默认 `inherit`）。声明后，sidecar 将作用域附加到委托的
`tools.execute` 调用上，host-core 在该模式下裁决每个调用，而不是使用
会话的有效权限模式。该作用域只是权限模式的覆盖：契约模式的硬拒绝和
外部路径门禁（§4.1）仍然生效，因此 `accept-edits` 会在工作区和
scratch 根内自动允许 `Write`/`Edit`，而所有其他调用 — 包括 Bash — 仍然
遵循会话模式。作用域下的 `auto` 与会话的 `auto` 模式行为相同。
未声明作用域的定义与此功能存在之前完全一致。
