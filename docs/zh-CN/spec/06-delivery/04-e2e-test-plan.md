...
- **先决条件**：一个会话可以产生故意延迟的第一个
  令牌和流响应。
- **步骤**： 1) 发送普通文本并在助理文本、思考或之前停止
  工具行开始。 2）确认用户行被撤消，文本返回到
  输入框。 3）再次发送，等待部分输出，然后停止
  流。 4）观察抄本和输入框。
- **预期**：未答复的发送将被撤消并恢复其草稿。的
  流发送停止，保留部分响应且没有草稿
  恢复或重复用户轮流。该会话仍然可用。
- **链接规格**：`03-runtime/02-agent-runtime.md`
- **接受**：C（中止）
...
- **先决条件**：已配置提供商；会话 A 正在流式传输很长的时间
  响应（composer 显示 stop/abort 控件）。
- **步骤**： 1) 当 A 仍在直播时，单击“新任务”/“新聊天”。 2）
  观察新会话的输入框。 3) 输入提示并在 A 时发送
  继续在后台播放。 4）让A完成并观察
  又是新会议的输入框。
- **预期**：新会话立即显示空闲的发送控件
  （绝不是卡住的 stop/abort 控件）并且其文本区域已启用；提示
...
- **步骤**： 1) 打开聊天路径。 2) 检查顶部的 46px 栏
  交谈区。 3) 确认其显示简洁的 session/task 标题，
  模型选择器和新任务/搜索/命令
  动作按钮；确认
侧边栏切换**仅在侧边栏折叠时出现**（展开时，
  ...
  插件或设置路由并检查同一顶部区域。
- **预期**：在聊天路径上，对话顶部栏以其
  标题、模型选择器和操作；它没有 Agent|Plan|Goal 模式控制。的
  左侧输入 Composer 芯片拥有活动会话的 Agent/Plan/Goal 开关。的
...
#### E2E-089：顶栏模型选择器向下打开并切换模型

- **先决条件**：聊天路线激活；提供商已配置。
- **步骤**： 1) 单击顶部栏中的模型选择器。 2）确认下拉菜单
  从酒吧向下打开。 3) 选择不同的 provider/model。 4）打开
  命令面板或应用程序菜单中的设置（顶部栏不再
  有一个设置操作按钮）。
- **预期**：菜单列出了具有默认模型的已启用的可运行提供程序
  并向下打开（固定在顶部栏上）；选择更新活动
  会话模型；通过命令 palette/menu 打开设置。模型触发器
  省略长 ID。
- **链接规格**：`04-ux/08-component-spec.md`（§2，模型下拉列表），
...
- **先决条件**：网络可访问 `cnb.cool`。
- **步骤**：1) 打开设置 → 扩展。 2) 将「插件市场来源」从 GitHub（官方）切换到镜像（cnb.cool）。 3) 打开扩展 → 市场。 4) 安装一个插件。
- **预期**：切换后自动触发刷新并报告新的插件数量；来源行与当前来源提示都显示 cnb.cool 目录地址；安装从镜像下载安装包并通过 shasum 校验。切回官方可恢复 GitHub 来源。选择自定义地址但留空时回退到官方默认地址，而不是空端点。
- **链接规格**：`07-plugins/07-plugin-marketplace.md`
- **接受**：G（远程市场来源）
- **状态**：已记录/涵盖 host-core 单元
...
- **步骤**： 1) 重新启动并检查标题栏和应用程序菜单；确认
  ...
  URL 预览和完整的命令工件。 3）打开header的统一
  上下文菜单：验证四个工具以固定顺序出现一次，并且处于活动状态，
  打开-非活动和关闭状态，并且出现转录打开的资源
  仅在第二部分。 Open/select 每个工具都带有指针和键盘，
...
- **前提**：工作区已打开；成功完成的命令
  神器存在。
- **步骤**：1) 激活已完成的命令工件以创建终端，
  然后运行 `pwd` 和 `ls`。 2) 切换到另一个工件选项卡并返回。 3）
  通过另一个命令工件折叠并重新打开。 4) 拖动调整大小
  面板并切换 light/dark 主题。 5) 运行 `exit`。 6) 通过以下方式重新启动
  覆盖按钮。 7) 退出应用程序并检查孤立 shell。
- **预期**：shell 作为登录名在工作区目录中启动
  带有256色TERM的外壳； output/scrollback 保留选项卡开关和
  面板关闭（重新连接相同的 PTY）；调整列大小而不会出现乱码；
  主题切换重新着色终端； `exit` 显示已结束的会话
  重新启动的覆盖层会启动一个新的 shell；应用程序退出会杀死所有 PTY；与
  没有工作区，选项卡显示其空状态，并且没有 PTY 生成。
...
#### E2E-060d：助理元芯片、上下文检查器和重试操作

- **前提**：完整的辅助消息包含modelId和token
用法;另一条已完成的助理消息有内容但没有用途。
- **步骤**： 1) 打开会话。 2) 将鼠标悬停在已完成的辅助回合上
  用法，然后使用键盘将其焦点放在上下文检查器触发器上。 3）检查
  面板的剩余令牌标题、已用百分比和计量、
  pi-ai-resolved 模型上下文窗口大小，精确 input/output/cache/reasoning
  故障，报告缓存读取元数据时的缓存命中率，已完成
  生成 tokens/s 值、源徽章以及每个
  独特工具类型的聚合调用计数和估计的 argument/result 令牌
  分配。 4）
  滚动文字记录，直到触发器接近顶部、底部和
  右视口边缘，并在面板打开时调整窗口大小。 5）点击
  空闲时重试该回合。 6) 确认回合不使用仍优惠
  重试并忽略检查员。
- **预期**：模型徽章和紧凑的上下文检查器出现在已完成的情况下
  助手在数据存在时应答；触发器显示剩余容量并且
  低空间 warning/error 状态，而悬停和键盘焦点会暴露
  相同的完整令牌面板。该小组的确切提供商和估计工具
  来源明显区分，其缓存命中率采用缓存提示
  令牌除以所有报告的提示令牌（缓存加上未缓存）；当
  缓存读取元数据不存在，速率被省略而不是推断。其
  context-window Total 与代理运行时使用的模型元数据匹配；
  工具行按首次看到的顺序显示每种独特的工具类型，重复聚合
  呼叫，包括呼叫次数和累积持续时间，并明确标记其
  估计。一代人
速率仍然是已完成的回合值，并且在流式传输期间不会更新；
  重试重新发送最近的先前用户提示，并在回合时被禁用
  正在运行；门户面板在视口中保持完全可见，
  从未被文字记录滚动剪辑，并在之后跟随触发器
  滚动或调整大小；文案仍然排除思考文本。
- **链接规格**：`04-ux/08-component-spec.md`，
  `04-ux/10-workbuddy-benchmark-ux.md`、`03-runtime/01-ipc-protocol.md`
- **验收**：C（聊天流），质量
- **里程碑**：M5
- **状态**：单位覆盖（`transcript-style.test.mjs`，
  `context-usage.test.mjs`，运行时使用映射）；完整场景草稿
...
- **步骤**：1) 在顶栏模型选择器中选择 `claude-opus-5`。 2）打开
  思维选择器。 3) 开始一个短回合并检查 sidecar 模型
快照/请求元数据（或单位等效解析路径）。
...
- **预期**：该软件包仅包含一个捆绑代理 sidecar，即
  目标本机 Rust 主机和 `node-pty`，并且仅配置了 Chromium 区域设置
...
- **先决条件**：项目绑定的 Agent 会话具有可写工作区；
  ...
  工具批次；第二次编辑可能会变得过时或不明确 `old_string`
  背景； Bash 命令可以返回带有诊断信息的非零退出代码。
- **步骤**：
  1. 启动一个任务，为同一会话发出两个突变，同时还
     发出独立的 read/search 调用。
  2. 在第一个突变运行时检查工具计时和转录本。
  3. 强制第二个 `Edit` 使用不再存在的上下文，然后
     允许代理重新读取文件并从当前内容重试。
  4. 运行以非零值退出的 Bash 命令并检查其工具结果
     内联状态。
  5. 对出现在两个位置的 `old_string` 重复此操作。
...
- **预期**：
  - Read/search 调用可能会重叠，但一次只执行一个 Agent/`Write`
    ...
    等待时的变异槽。
  - 陈旧且不明确的编辑会在不更改文件的情况下失败并返回
    `TOOL_FAILED` 消息指示代理重新读取并提供
    独特的当前背景。
  - 非零 Bash 命令被标记为失败，同时保留其退出代码，
    用于代理和诊断的 stdout 和 stderr。
- 重试执行一次新读取并对当前文件进行操作；一个
    第二个相同路径不匹配或失败的 shell patch 命令返回
    终止工具结果，停止变异工作流程，并且不
    重复修改旧的补丁工件或其块标头。
...
#### E2E-096：就地恢复一个临时提供程序流终止

- **先决条件**：项目绑定的 Agent 会话使用确定性
  发出部分辅助流的提供商装置，终止一次，
  然后下一个请求成功；第二场比赛可以终止两次；
  计时日志已启用。
- **步骤**：
...
4. 重新加载会话并验证是否只有已完成的响应或
     单终端故障助手依然耐用。
- **预期**：
  - `terminated` 被分类为 `STREAM_FAILED`。
  - 第一个流后瞬态故障等待一个可中止的界限
    退避，从模型上下文中删除失败的助手，并重试一次
    没有重复的助手气泡或终端错误通知。
  - 恢复的回合发出一个终端生命周期并保持相同的可见性
    助理消息 ID。时序日志记录`outcome=retry`和最终的
    结果。
  - 第二次故障发出一个终端 `STREAM_FAILED` 辅助错误并且
    生命周期事件。可用的详细信息包括阶段、流计时、提供商
    状态和 `retryAttempt`，无需凭据或不受限制的提供商
    身体。
...
- **里程碑**：M5
- **状态**：单位覆盖（`agent-errors.test.ts`、`runtime.test.ts`）；满
  provider/UI 旅程草案
...
- **先决条件**：应用程序正在项目中使用 Agent 会话运行，并且
  可用的家庭输入框。操作系统剪贴板包含一个文本片段，一个或
  更多本地文件（包括带空格的文件名）和图像
  单独的粘贴尝试。记录应用程序数据目录和会话 ID。
- **步骤**：
  1. 仅将文本粘贴到编辑器中，并确认文本是由编辑器插入的
本机文本区域路径。
...
| 验收 | 应用场景 |
|---|---|
| A — 应用程序启动 | E2E-001、E2E-002、E2E-003、E2E-004、E2E-067、E2E-076、E2E-079、E2E-092、E2E-097 |
| B——模型配置 | E2E-005、E2E-006、E2E-007、E2E-038、E2E-050、E2E-052、E2E-055、E2E-066、E2E-080、E2E-082 |
| C — 对话和直播 | E2E-008、E2E-008a、E2E-009、E2E-010、E2E-011、E2E-011a、E2E-011b、E2E-031、E2E-040、E2E-047、E2E-048、E2E-048A、E2E-049、E2E-052、 E2E-053、E2E-054、E2E-055、E2E-059、E2E-059a、E2E-060c、E2E-060d、E2E-061、E2E-061a、E2E-062、E2E-064、E2E-065、E2E-068、E2E-071、 E2E-073、E2E-074、E2E-075、E2E-081、E2E-083、E2E-084、E2E-086、E2E-087、E2E-088、E2E-089、E2E-090、E2E-094、E2E-095、E2E-096、 E2E-097、E2E-098、E2E-099、E2E-102、E2E-102a、E2E-102b、E2E-106、E2E-109、E2E-111、E2E-114、E2E-116、E2E-117、E2E-118、E2E-119、 E2E-120、E2E-121、E2E-代理-001 |
| D——工作区 | E2E-012、E2E-013、E2E-022B、E2E-024I、E2E-047、E2E-049、E2E-057、E2E-058、E2E-060、E2E-068、E2E-075、E2E-078 |
| E——工具和权限 | E2E-008a、E2E-014、E2E-015、E2E-016、E2E-017、E2E-018、E2E-019、E2E-024I、E2E-024K、E2E-040、E2E-049、E2E-074、E2E-093、E2E-097、 E2E-099、E2E-100、E2E-101、E2E-102、E2E-103、E2E-105、E2E-106、E2E-107、E2E-111、E2E-112、E2E-113、E2E-114、E2E-115、E2E-116、 E2E-119、E2E-121、E2E-122、E2E-123 |
...
| G——插件 | E2E-022、E2E-022A、E2E-022B、E2E-022C、E2E-023、E2E-024、E2E-024B、E2E-024C、E2E-024D、E2E-024E、E2E-024F、E2E-024G、E2E-024H、 E2E-024I、E2E-024J、E2E-024K、E2E-024L、E2E-024M、E2E-024N、E2E-024O、E2E-024P、E2E-025、E2E-026、E2E-105、E2E-117、E2E-120、E2E-122、E2E-123 |
| H——诊断 | E2E-027、E2E-031、E2E-034、E2E-042、E2E-096、E2E-098、E2E-104、E2E-107、E2E-108、E2E-109、E2E-110、E2E-113、E2E-115、E2E-116、 E2E-118、E2E-121 |
| 安全性 | E2E-028、E2E-029、E2E-030、E2E-024J、E2E-024K、E2E-024M、E2E-049、E2E-068、E2E-086、E2E-105、E2E-106、E2E-107、E2E-108、E2E-109、 E2E-110、E2E-112、E2E-113、E2E-115、E2E-116、E2E-117、E2E-119、E2E-121、E2E-122、E2E-123 |
...
### US-UI-21 顶栏模型菜单配置 pi
- 与提供商 A/model A 创建会话，然后打开顶栏模型菜单。
- 期望启用、可运行 provider/default-model 对和 Agent 条目。的
  模型触发器仅显示模型 ID；能力门控思维触发器是
  放置在 Agent / Plan / Goal 旁边，而不是嵌套在模型菜单中。
...
- 空输入框显示 PI-Desktop 占位符副本：EN `Ask PI-Desktop to do anything`、zh-CN `向 PI-Desktop 下达任意指令`。
- 占位符墨水在浅色和深色浮动板上清晰可见。
...
- 铁路订单完全是基础知识、全局 AI/AI、快捷方式、说明、模型
  配置、导入、项目存档和信息；
  没有 Personal/Integrations/Coding 组标题、插件重复、
  或占位符目的地。
- 基本内容：大标题和一张带有工作信息的**外观**卡
  system/light/dark 控件和 **默认** 卡。 全局AI持有权限
  上下文管理；快捷键包含键盘快捷键卡。
  文件打开目标、语言
  覆盖、菜单栏行为和底部面板行为不存在，直到
  存在主机支持的实现。
- 模型配置包含提供商工作室英雄，默认mode/model，回车发送
  交换机，以及带有添加提供商对话框的基于卡的提供商管理。
...
### US-UI-61 助理上下文检查器 + 重试（D103、D184）
- 完成报告使用情况的助理回合。
- 答案下方会有模型徽章和紧凑的上下文检查器。的
  触发器显示剩余上下文百分比；悬停或键盘焦点显示
  used/remaining/window 代币，确切的 input/output/cache/reasoning 用法，
  生成 tokens/s，以及每个独特工具类型的聚合调用计数，
  argument/result 足迹、份额和持续时间。面板标签提供商
  报告的总计和估计的工具行。
- 将触发器移动到每个视口边缘附近，并在面板滚动或调整大小时
  已开放；期望车身水平覆盖层翻转、夹紧并完全保持
  可见，而不是被脚本滚动容器剪切。
- 将鼠标悬停在操作行并单击“重试”；最近的前面的用户提示是
  重新发送。
