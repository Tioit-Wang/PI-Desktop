...
- 仅修饰符按键和 IME composition/229 按键永远不会调度
  命令。重复的按键事件不会重复遍历目的地
  历史；每个 back/forward 和弦每次物理按下最多前进一次。
...
低级键盘钩子，消耗系统菜单和弦并发出
  ...
  专注。如果钩子无法被调用，聚焦窗口后备仍然可用
  安装。自定义绑定继续使用 Electron 的全局快捷方式 API。
  启动器始终在最靠近指针的显示屏上打开。
...
- 单击或双击 PI-Desktop 托盘图标，从其中选择“显示”
  ...
  窗口。
- 托盘菜单使用活动的 English/zh-CN shell 语言环境进行本地化，并且
公开 Show PI-Desktop 以及显式退出 PI-Desktop 操作。戒烟使用
  现有的有序关闭路径；关闭窗口仍然是退出操作
  而不是隐式托盘隐藏。
...
- 脚本加载开始，无需等待旧的被取代的选择。
  ...
  导航历史记录和工作面板上下文。
- 工作区对齐后可能会出现温暖的转录快照，并且
  总是重新验证。冷切换或 React 延迟切换会保留之前的切换
  完整的转录本是一个暗淡的、非交互式的稳定框架，带有薄的
  进度跟踪和 `aria-busy`；它从不合并目标会话 ID
  与上一会话的消息。准备好后，目的地将绘制在
  这是它最后的记录，没有创下历史之巅，也没有空置的闪光。
- 选择项目范围的对话会激活其项目作为
  商店自有精选交易。选择临时对话将清除
...
3. Electron 向每个实时渲染器发出 `notification.changed`，以便响铃
   徽章和当前打开的收件箱刷新。
4. 如果主窗口获得焦点，则不会出现其他表面。如果是的话
   支持未聚焦和本机通知，Electron 显示一个
   从事件类型和会话标题派生的平台通知。开
   Windows，该横幅归因于规范的 PI-Desktop
   AppUserModelID 与 NSIS 包和任务栏标识共享。
...
- 文件资源使用规范化路径作为标识。审查、终端和
  浏览器是单例的；重复触发器保留资源顺序并且
  激活现有资源。
- 打开后，面板的统一上下文触发器将锚定在标题的左侧
并打开一个下拉菜单。它的顶部列出了四个工具——审查、
  终端、浏览器、文件 — 按固定顺序，每一行都有自己的打开状态
  状态，一旦打开，它自己关闭控制。第二部分出现在
...
- 紧凑的更新通知仅出现在主窗格的右上角安全区域中
  ...
  `downloaded` 等阶段再次出现。
- 当 Main 附加发现版本的双区域设置产品说明时
  （`UpdateState.releaseNotes`，D164），通知和设置→信息更新
  行在状态消息下显示紧凑的“新增内容”列表。笔记来了
  从产品 UI 选择的已发货 EN/zh-CN 变更日志目录中
  语言环境 — 绝不来自渲染器提供的提要或远程 URL。缺少目录
  条目省略了该部分；区域设置更改重新解析注释，无需新的
  检查。
...
转录内容。它基于终端代理事件，而不是超时
  ...
  现有的成绩单和消息范围的审查卡仍待完成
  证据。
- 失败副本表明现有工作仍然可用。重试重新发送
  通过现有重试路径的最新用户提示；继续退货
  将注意力集中在输入框身上而不改变抄本。
...
- 当面板弹性分配打开时，MainChat 会持续回流或
  ...
  可能会回流低于其 360px 目标。
- 本机窗口和侧边栏调整大小不会夹紧或重写面板。本地人
  边缘仅通过回流调整 MainChat 的大小。
...
## 8a。输入框自动完成和剪贴板文件（D123–D125、D197）
...
- 文件结果通过仅渲染叶名称（带有
  目录尾随 `/`）。完整的相对路径仍然可用
行工具提示和可访问的名称。接受文件会创建一个压缩文件
  由完整的 `entry.path` 支持的参考；接受目录保留
  文本区域中的完整文字路径，以便可以继续进行更深入的补全。
