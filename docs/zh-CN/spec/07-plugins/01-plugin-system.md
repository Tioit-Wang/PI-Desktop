...
四个选项卡，因为这三种创建方式完全不同——
安装、配置、编写——并且一个合并列表必须隐藏这一点
在最低公分母行后面：

| 选项卡 | 内容 |
|---|---|
| 已安装 | 插件，分组：需要注意、更新、活动、禁用 |
| MCP | 用户自己的 MCP 服务器 (D193) |
| 技能 | 用户自己的技能文档（D194） |
| 市场 | 浏览并安装 |
...
- **从 JSON 导入**是主要操作：粘贴的 `mcpServers` 块是什么
  用户手头有。列出不良条目时会附有原因，但并非致命的 (D193)。
- 每行的前导字形兼作连接灯 - 空闲、连接、
  准备好了，失败了——所以“这可行吗”是第一句话。
- 编辑器表具有 stdio 和 HTTP 传输卡、用于 env 的 key/value 行或
  标题，以及一个**测试连接**按钮，用于报告它找到的工具名称。

### 12. 3 技能选项卡

- 创建写入 `SKILL.md`； import 将现有文档复制到。
- 描述是必需的，位于正文上方，因为它是唯一的
  进入提示符的部分（D174、D194）。
