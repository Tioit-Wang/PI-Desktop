...
**今天实现：**运行时（`apps/desktop/electron/main/plugin-runtime.ts`）调用`onLoad`（当在load/enable上加载插件时）和`onUnload`（在unload/disable/reload上调度到插件进程，预算5s，然后进程停止）；卸载会删除插件注册的命令和工具。下面的其他挂钩在 API 中声明但尚未触发。
