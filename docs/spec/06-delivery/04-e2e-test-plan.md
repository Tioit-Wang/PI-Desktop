...
- **Steps**: 1) Set the app language to English and open a panel whose manifest declares localized `ui.title.en` and `ui.title.zh-CN`; confirm the native window/launcher identity remains available without a host-rendered title. 2) Set the app language to Simplified Chinese and reopen the panel; confirm the panel content remains plugin-owned. 3) Open the panel on macOS, Windows, and Linux; confirm the same frameless 46px safe area, fixed top-right capsule, and exactly three accessible controls. 4) Exercise minimize, maximize, restore, close, keyboard focus, light/dark themes, page-defined light/dark backgrounds, and reduced motion on every platform. 5) Render a plugin-owned titlebar/toolbar; verify fixed/sticky UI uses `--pi-plugin-titlebar-height`, its interactive controls use `no-drag`, and the plugin surface does not sit beneath the host capsule. 6) Reopen a minimized panel. 7) Invoke panel bridge APIs (`ui.showToast`, optional fs/net with grants).
- **Expected**: Panel runs in its sandboxed window/partition; all three platforms use one host-owned frameless chrome contract with no native traffic lights, host-rendered title, development reminder, or application menu; the capsule stays fixed at the top-right, contains minimize/maximize-or-restore/close, follows the plugin page's surface/text colors, and never forces a black surface onto a light page. Plugin content starts below the transparent safe area without losing its original top padding, while the plugin owns its title and toolbar; the host drag strip remains usable and plugin toolbar controls remain clickable. Reopening restores the existing panel; bridge calls remain permission-checked and the host remains stable on panel close.
- **Specs linked**: `03-runtime/01-ipc-protocol.md`, `04-ux/07-ui-design-system.md`, `07-plugins/01-plugin-system.md`, `07-plugins/03-plugin-api.md`, `07-plugins/04-plugin-security.md`, ADR 0081, ADR 0082, ADR 0092
