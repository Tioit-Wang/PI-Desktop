...
| D179 | Message-scoped inline review cards | **Every successful workspace Write/Edit tool row may render one compact InlineReviewCard immediately after that row inside the same activity disclosure when the current workspace diff still contains the path returned by the tool. The card shows localized added, modified, deleted, renamed, or untracked status, addition/deletion totals, and expandable hunks in place. It is not a transcript-bottom launcher and does not carry session ownership state; failed, scratch, clean, non-Git, and missing-workspace results render no card. A background session's card remains with its originating transcript and never appears in the currently visible session. The full Review tab remains the all-files current-worktree view and shares the renderer's workspace-keyed diff.** | The old global entry detached the review action from the tool that caused the change and forced the user to reconstruct which file mattered. Keeping one disclosure beside the originating row follows WorkBuddy's artifact grammar while preserving the existing bounded Git diff and Review panel. |
