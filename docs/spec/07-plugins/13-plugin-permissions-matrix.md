...
| `fs.read` | medium | `fs.readText` / `fs.glob` / `fs.requestDirectory` | Granted at install, bounded by `manifest.fs.read` | Whole-tree scope is allowed; egress is what makes a read dangerous and it is closed separately (§2A) |
...
[04-plugin-security.md](04-plugin-security.md) §6 and §8.1, and ADR 0087.
