...
The Composer provides a one-shot `Enhance prompt` action for a non-empty draft.
The action rewrites only the draft text with the model currently displayed in
...
The sparkle action is enabled only when all of the following are true:
