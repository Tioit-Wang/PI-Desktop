...
### 3.5 Extensions
The Extensions destination uses a compact, two-tier tool header: a quiet
extension mark and medium-weight page title share the first row with the
contextual primary action and borderless overflow menu; the five-part tab row —
Installed / MCP / Skills / Subagents / Marketplace — uses restrained active
surfaces and relevant tab counts. It does not render a separate numeric
overview band (D202 amends D196, which amends D169). The title row and tab row
use hairline separators and restrained control heights so the Windows frameless
titlebar remains visually separate from the page controls; the top-right action
rail must remain below the native window control band and retain its no-drag hit
area.
